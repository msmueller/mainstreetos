...
// ------------------------------------------------------------
// Phase 12.13 — AI Draft pipeline (Offering Memorandum, pre-NDA)
// Broker clicks "Generate OM Draft" on a deal page. This route:
//   1. Verifies auth + broker access to the seller_listing
//   2. Calls Claude Sonnet via agent-om-draft to produce markdown
...
import { generateOmDraft, type OmListingInput } from '@/lib/agents/agent-om-draft'
...
export async function POST(
  ...
  try {
    ...
    // 2. Fetch seller_listing — use the RLS-scoped client. If the user can
    //    read the row, they are either a broker, owner, primary contact, or
    //    have parent access. That's exactly the "broker or listing-scoped"
    //    gate we want for generating an OM draft.
    ...
    // 4. Build the OM input from the listing row
    const custom = (listingRow.custom_fields ?? {}) as Record<string, unknown>
    const omInput: OmListingInput = {
      ...
      generic_title: typeof custom.generic_title === 'string' ? custom.generic_title as string : null,
    }

    // 5. Generate the OM markdown via Claude
    const draft = await generateOmDraft(omInput, brokerLicense)
    ...
    const pageTitle = `${businessDisplay} — Offering Memorandum — ${todayYmd()}`
    ...
    const { data: draftRow, error: draftErr } = await admin
      ...
      .insert({
        ...
        rationale: `Generated by agent_om_writer (${draft.tier} → ${draft.model}) on ${todayYmd()}.`,
    ...
    try {
      await admin.from('thoughts').insert({
        content: `Generated OM Draft for ${businessDisplay} (${listingRow.industry ?? 'unknown industry'}). Asking ${listingRow.asking_price_usd ? '$' + Number(listingRow.asking_price_usd).toLocaleString() : 'TBD'}. Notion: ${notionResult.pageUrl}`,
        metadata: {
          type: 'reference',
          source: 'om_writer_agent',
          topics: ['om', 'offering memorandum', listingRow.industry?.toLowerCase() || 'business', 'ai draft'],
          people: [],
          listing_id: dealId,
          notion_page_id: notionResult.pageId,
          notion_page_url: notionResult.pageUrl,
